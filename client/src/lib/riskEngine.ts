/**
 * Client-side Risk Intelligence.
 */

import type { Parameters } from '@/types'
import type { ClimateContext, RiskScoreBreakdown } from '@/types'
...
export function computeRiskScore(
  ...
  const overall = crack * 0.35 + weather * 0.25 + automation * 0.2 + cycle * 0.2
  return {
...
function crackProbability(params: Parameters, climate: ClimateContext | null): number {
  let p = 0.1
  if (params.curing_method === 'ambient') p += 0.2
  if (params.region === 'north_india' && climate && (climate.temp < 12 || climate.temp > 38)) p += 0.15
  if (climate && climate.humidity > 90) p += 0.1
  if (params.mix_strength_mpa < 35) p += 0.1
  return Math.min(1, p)
}

function weatherVolatilityRisk(climate: ClimateContext | null): number {
  if (!climate) return 0.2
  const impact = (climate.climate_impact_score ?? 50) / 100
  return Math.min(1, impact * 1.2)
}

function automationFailureRisk(level: string): number {
